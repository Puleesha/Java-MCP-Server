...
#    mv target/$(./mvnw help:evaluate -Dexpression=project.artifactId -q -DforceStdout)-$(./mvnw help:evaluate -Dexpression=project.version -q -DforceStdout).jar target/baseline-server.jar
#
#
#################################################################################
#
...
#COPY --from=package build/target/baseline-server.jar baseline-server.jar
#
#EXPOSE 6565
#
#ENTRYPOINT [ "java", "-jar", "baseline-server.jar" ]
